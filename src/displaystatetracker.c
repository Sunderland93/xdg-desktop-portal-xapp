...
struct _DisplayStateTracker
{
    ...
    OrgCinnamonMuffinDisplayConfig *proxy;
                      ...
                      gpointer user_data)
{
    ...
    if (!org_cinnamon_muffin_display_config_call_get_current_state_finish (tracker->proxy,
        NULL,
        &monitors,
        &logical_monitors,
        NULL /* props */,
...
{
    org_cinnamon_muffin_display_config_call_get_current_state (tracker->proxy,
                                                               tracker->cancellable,
...
static void
on_monitors_changed (OrgCinnamonMuffinDisplayConfig *proxy,
                                  ...
                                  gpointer      user_data)
{
    ...
    proxy = org_cinnamon_muffin_display_config_proxy_new_for_bus_finish (result, &error);
    if (!proxy)
      {
        g_warning ("Failed to acquire org.cinnamon.Muffin.DisplayConfig proxy: %s",
                   error->message);
        return;
      }
                                 ...
                                 gpointer user_data)
{
    ...
    org_cinnamon_muffin_display_config_proxy_new_for_bus (G_BUS_TYPE_SESSION,
